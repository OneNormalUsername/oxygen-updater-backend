...
// When true, auth using $auth_users and $readonly_users defined below (development tip!)
...
$root_path = getenv('NEWS_IMAGES_PATH');

// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
$root_url = getenv('NEWS_IMAGES_RELATIVE_URL');
...
// Authentication URL of the AdminPortal API
$auth_url = getenv('NEWS_IMAGE_UPLOADER_AUTH_URL');
...
if (empty($auth_users) && $use_in_memory_auth) {
    $use_auth = false;
}
...
function login($user, $password) {
    ...
    if ($status_code === 401) {
        ...
        error_log("Error when logging in user for news image uploading. HTTP status code: " . $status_code);
        return null;
    }
...
// Original h3k code
if ($use_auth) {
    if (isset($_SESSION['logged']) && (!$use_in_memory_auth || isset($auth_users[$_SESSION['logged']]))) {
        // Logged In, go to page
    } elseif (isset($_POST['fm_usr'], $_POST['fm_pwd'])) {
        // Logging In
        if ($use_in_memory_auth) {
            sleep(1);
            if (isset($auth_users[$_POST['fm_usr']]) && md5($_POST['fm_pwd']) === $auth_users[$_POST['fm_usr']]) {
                ...
                unset($_SESSION['logged']);
                fm_set_msg('Invalid username or password', 'error');
                fm_redirect(FM_SELF_URL);
            }
        } else {
            $allowed = login($_POST['fm_usr'], $_POST['fm_pwd']);
            if ($allowed === true) {
                $_SESSION['logged'] = $_POST['fm_usr'];
                fm_set_msg('You are logged in');
                fm_redirect(FM_SELF_URL . '?p=');
                ...
                unset($_SESSION['logged']);
                fm_set_msg('Technical error when logging in. Please contact an admin', 'error');
                fm_redirect(FM_SELF_URL);
            }
        }
    } else {
        ...
        <div class="path login-form">
                <img src="https://oxygenupdater.com/img/app_icon-min.png" alt="Oxygen Updater" style="width:100px; margin:20px;">
...
<head>
<meta charset="utf-8">
<title>Oxygen Updater | News Image Manager</title>
<meta name="Description" CONTENT="News Image Manager">
<link rel="icon" href="https://oxygenupdater.com/favicon.ico">
<link rel="shortcut icon" href="https://oxygenupdater.com/img/app_icon-min.png" type="image/png">
...
<meta name="Description" CONTENT="News Image Manager">
<link rel="icon" href="https://oxygenupdater.com/favicon.ico">
<link rel="shortcut icon" href="https://oxygenupdater.com/img/app_icon-min.png">
<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.css">
